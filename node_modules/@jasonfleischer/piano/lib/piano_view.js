...
class PianoView {
	...
	clearHover() {
		this.black_keys_drawing_hover_canvas.getContext("2d").clearRect(0, 0, this.WIDTH, this.HEIGHT);
		this.white_keys_drawing_hover_canvas.getContext("2d").clearRect(0, 0, this.WIDTH, this.HEIGHT);
	}
	clearHoverNote(note) {
	...
	drawHoverNote(note, color = undefined){
		if(note == undefined) {
			log.e('note is undefined')
			return
		}
		...
		if(color == undefined){
			color = note.note_name.is_sharp_or_flat ? "#aaaaaaaa" : "#33333333";
		}
		let key = this.midi_value_to_piano_key_map[note.midi_value];
		key.draw(ctx, color);
	}
